 ...
 */
public class SafiumBaabPremium implements Player {

    final int maxDepth;

    public SafiumBaabPremium() {
        this(3);
    }

    public SafiumBaabPremium(int maxDepth) {
    ...
    public String getPlayName() {
        String name = "Safium Baab Premium";
    ...
    private double scoreFutureState(GameState startState, GameState futureState) {
        ...
        double vulnerablilty = 0; 
        for (int i = 0; i < 6; i++) {
            int seeds = futureState.getStones(us, i);
            if(seeds == 1 || seeds == 2) {
                vulnerablilty += seeds;
            }
        }
        
        //if(opponentDifference > ourDifference) return -opponentDifference;
        return ourDifference - opponentDifference - vulnerablilty;
    }
}
